...
def find_exit(start_E: E, end_pos, ob_states, dimensions):
    ...
    while queue:
        ...
        if cur_tick - last_tick >= 0.1:
            ...
            print()
            last_tick = cur_tick
        if (not tuple(cur_E.pos) in obstacles and cur_E.y >= -1 and
...
def main():
    ...
    time = find_exit(E(np.array(start_pos)), end_pos, ob_states, (width, height))
    if not time:
        print(f'End not found')
    else:
        print(f'Time to end: {time} minutes')
